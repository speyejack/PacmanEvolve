...
function initalizePool()
	...
	console.log("Generation: "..pool.generation..":"..#pool.species..":"..pool.maxFitness)
...
function sameSpecies(s1, s2)
	...
	local diff = (DeltaExcess * excess / n) + (DeltaDisjoint *  disjoint/ n) + (weight / count)
...
function newGeneration(pool)
	-- Rank, remove bad species, cut popluation, replace bad people
	local realMax = 0
	...
	pool.currSpecies = 1
	pool.progression = 0
	console.log("Gen:"..pool.generation.." #S:"..#pool.species.." maxF:"..pool.maxFitness..string.format(" rMaxFit:%d",(realMax))..string.format(" t:%d",(os.clock() - pool.time)))
	pool.time = os.clock()
	savePool("backup."..pool.generation.."."..SaveName..".pool", pool)
	startPoolState(PoolStateName,pool)
end 
...
function playTop(level, pool)
	...
	console.write("TopF: "..topFitness.. " RealF: "..pool.species[topSpecies].genomes[topGenome].fitness)
	if topFitness >= pool.maxFitness then
		client.speedmode(100)
		testGenome(level, pool, pool.species[topSpecies].genomes[topGenome])
		console.log(" FoundF: "..pool.species[topSpecies].genomes[topGenome].fitness)
...
function loadPoolState(filename)
	...
	local pool = loadPool(temp)
	local line = file:read("*line")
	local genome = getNextGenome(pool)
	while line ~= nil do
...
function testGenome(level, pool, genome)
	...
	local frame = emu.framecount() % 5
	
	local outs = nil
	generateNetwork(genome)
	savestate.load(StateName..".state")
	local inps = getInputs(level)
	local canvas = createGUI(pool, genome)
	outs = evaluateNetwork(genome, inps)
	while isTesting do
		if emu.framecount() % 5 == frame then
			inps = getInputs(level)
			outs = evaluateNetwork(genome, inps)
			genome.fitness = getScore()
...
function temp(pool)
	local p1 = basicGenome()
	local link = newLink()
	local inno = newInnovation()
	link.innovation = inno
	link.into = math.floor((Inputs / 2) - 1)
	link.out =  MaxNodes + 2
	link.weight = 1
	p1.links[1] = link
	nodeMutate(p1)
	
	addToGeneration(pool, p1)
	generateNetwork(p1)
	p1.fitness = 100
	
	
	local p2 = basicGenome()
	local link = newLink()
	link.innovation = inno
	link.into = math.floor((Inputs / 2) + 2*InputRadius + 1)
	link.out =  MaxNodes + 2
	link.weight = 1
	p2.links[1] = link
	nodeMutate(p2)
	
	addToGeneration(pool, p2)
	generateNetwork(p2)
	--createForm(p1)
	return p1,p2
end
...
local level = loadLevel(LevelName)
local pool = nil
